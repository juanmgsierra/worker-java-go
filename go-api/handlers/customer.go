...
func NewCustomerHandler(coll *mongo.Collection, rds *redis.Client) *CustomerHandler {
	return &CustomerHandler{Coll: coll, Redis: rds}
}
...
func (h *CustomerHandler) GetByID(ctx *gin.Context) {
	...
	if err == mongo.ErrNoDocuments {
		ctx.JSON(http.StatusNotFound, gin.H{"err": "customer not found"})
		return
	}
